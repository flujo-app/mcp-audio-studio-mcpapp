...
// Every literal below is something the package claims about itself: the CLI
// banner, the MCP server identity and the MCP App client identity. Each pattern
// must expose exactly two capture groups so "$1<version>$2" rebuilds the line.
const targets = [
  { file: "src/server/index.ts", pattern: /(console\.log\("mcp-audio-studio )\d+\.\d+\.\d+("\);)/ },
  { file: "src/server/index.ts", pattern: /(MCP Audio Studio )\d+\.\d+\.\d+(\r?\n)/ },
  { file: "src/server/mcp.ts", pattern: /(name: "MCP Audio Studio",\s*version: ")\d+\.\d+\.\d+(")/ },
  { file: "src/web/bridge.ts", pattern: /(\{ name: "MCP Audio Studio", version: ")\d+\.\d+\.\d+(")/ },
  { file: "tests/bridge.test.ts", pattern: /(\{ name: "MCP Audio Studio", version: ")\d+\.\d+\.\d+(")/ },
];
